...
vmanage_instance_id=$(vault kv get --field=vmanage_instance_id concourse/sdwan/$NAME/vmanage_2/vmanage_instance_id)
...
vol_id=$(vault kv get --field=vol_id concourse/sdwan/$NAME/vmanage_2/vol_id)
export vol_id=$vol_id
echo $vol_id
aws ec2 delete-volume --volume-id $vol_id
echo "Deleting the secondary and tertiary network cards"
#Get public eni id from vault
public_eni_id=$(vault kv get --field=public_eni_id concourse/sdwan/$NAME/vmanage_2/public_eni_id)
export public_eni_id=$public_eni_id
echo $public_eni_id
aws ec2 delete-network-interface --network-interface-id $public_eni_id --region $REGION
#Get cluster eni id from vault
cluster_eni_id=$(vault kv get --field=cluster_eni_id concourse/sdwan/$NAME/vmanage_2/cluster_eni_id)
export cluster_eni_id=$cluster_eni_id
echo $cluster_eni_id
aws ec2 delete-network-interface --network-interface-id $cluster_eni_id --region $REGION
