#!/usr/bin/env python
#This Script creates the vsmart from AMI and the key - split out the key
...
keypair_name=name
sg_name=name
ami_id=vSmart_ami_id

path = os.getcwd()
print(path)
os.listdir(path)

#Tag the Instance with the name vsmart
#aws ec2 create-key-pair --key-name MyKeyPair moved to aws prep

instance_type='c5.4xlarge'
outfile_deploy_vsmart='deploy-vsmart.json'
...
cmd_deploy_vsmart='aws ec2 run-instances --region' + " " + "{}".format(region) + " " + '--image-id' + " " + "{}".format(ami_id) + " " + '--instance-type' + " " + "{}".format(instance_type) + " " + '--subnet-id' + " " + "{}".format(subnetid_router) +  " " + '--security-group-ids' + " " + "{}".format(sgid) + " " + '--key-name' + " " + "{}".format(keypair_name) + " " + '--placement AvailabilityZone=' + "{}".format(az)
#cmd_deploy_vsmart='aws ec2 run-instances --image-id' + " " + "{}".format(ami_id) + " " + '--instance-type' + " " + "{}".format(instance_type) + " " + '--subnet-id' + " " + "{}".format(subnetid_router) +  " " + '--security-group-ids' + " " + "{}".format(router_sg_id) + " " + '--associate-public-ip-address' + " " + '--key-name' + " " + "{}".format(keypair_name)
#print(cmd_deploy_vsmart)

output = check_output("{}".format(cmd_deploy_vsmart), shell=True).decode().strip()
print("Output: \n{}\n".format(output))

with open(outfile_deploy_vsmart, 'w') as my_file:
    my_file.write(output)


with open (outfile_deploy_vsmart) as access_json:
    read_content = json.load(access_json)
    question_access_vsmart = read_content['Instances']
    replies_access_vsmart = question_access_vsmart[0]
    replies_data_vsmart=replies_access_vsmart['InstanceId']
    print(replies_data_vsmart)
    vsmart_instance_id=replies_data_vsmart #add this to the vars file
    print(vsmart_instance_id)
    vsmart_instance_id_var=('vsmart_instance_id=' + "'" + "{}".format(vsmart_instance_id) + "'")
    vsmart_int_ip_addr=replies_access_vsmart['PrivateIpAddress']
    print(vsmart_int_ip_addr)

with open(outfile_vars, 'a+') as my_file:
    my_file.write(vsmart_instance_id_var + "\n")


#Capture the instance_id and write to the var file or vault
vsmart_instance_id_var=('vsmart_instance_id=' + "'" + "{}".format(vsmart_instance_id) + "'")
print(vsmart_instance_id_var)
with open(outfile_vars, 'a') as my_file:
    my_file.write(vsmart_instance_id_var + "\n")


#tag the vsmart
#tag_vsmart='aws ec2 create-tags --resources' + " " + "{}".format(vsmart_instance_id) '--tags "'Key="[Name]",Value=vsmart'"
vsmart_tag_inst='aws ec2 create-tags --region' + " " + "{}".format(region) + " " + '--resources' + " " +  "{}".format(vsmart_instance_id) + " " + '--tags' + " " + "'" + 'Key="Name",Value=vsmart' + "'"
output = check_output("{}".format(vsmart_tag_inst), shell=True).decode().strip()
print("Output: \n{}\n".format(output))
...
cmd_vsmart_nic0_eni_id='aws ec2 describe-instances --instance-id' + " " + vsmart_instance_id + " " + '--query' + " " + 'Reservations[].Instances[].NetworkInterfaces[*].NetworkInterfaceId'
print(cmd_vsmart_nic0_eni_id)
output = check_output("{}".format(cmd_vsmart_nic0_eni_id), shell=True).decode().strip()
print("Output: \n{}\n".format(output))
...
cmd_associate_eip_router='aws ec2 associate-address --allocation-id eipalloc-0aca95ad8f1f8220f --network-interface-id' + " " + nic0_eni_id
print(cmd_associate_eip_router)
output = check_output("{}".format(cmd_associate_eip_router), shell=True).decode().strip()
print("Output: \n{}\n".format(output))

#tag the vsmart_nic0_eni_id
#write the eip to the vault



#Capture the private IP Address and write to the var file or vault
vsmart_int_ip_addr_var=('vsmart_int_ip_addr=' + "'" + "{}".format(vsmart_int_ip_addr) + "'")
print(vsmart_int_ip_addr_var)
with open(outfile_vars, 'a') as my_file:
    my_file.write(vsmart_int_ip_addr_var + "\n")


#Pole until the instance is created....
##!!HERE WE NEED TO POLL AND WAIT UNTIL THE INSTANCE IS IN AN INITIALIZED STATE -
#aws ec2 wait instance-status-ok --instance-ids vsmart_instance_id
cmd_check_instance='aws ec2 wait instance-running --instance-ids' + " " + vsmart_instance_id + " " + '--region' + " " + "{}".format(region)
output = check_output("{}".format(cmd_check_instance), shell=True).decode().strip()
print("Output: \n{}\n".format(output))



#Get the external public address assigned to the vsmart and write it to the var file or vault
outfile_vsmart_pub_ip='vsmart_pub_ip.json'
cmd_get_vsmart_pub_ip='aws ec2 describe-instances --region' + " " + "{}".format(region) + " " '--instance-id' + " " + "{}".format(vsmart_instance_id) + " " + '--query "Reservations[*].Instances[*].PublicIpAddress"'
output = check_output("{}".format(cmd_get_vsmart_pub_ip), shell=True).decode().strip()
print("Output: \n{}\n".format(output))
with open(outfile_vsmart_pub_ip, 'w') as my_file:
    my_file.write(output)

outfile_vsmart_pub_ip='vsmart_pub_ip.json'
with open(outfile_vsmart_pub_ip) as access_json:
    read_content = json.load(access_json)
    question_access = read_content[0]
    print(read_content)
    question_data=question_access[0]
    vsmart_pub_ip=question_data
    print(vsmart_pub_ip)


#Capture the External IP address and write it to the var file or vault
    vsmart_pub_ip_var=('vsmart_pub_ip=' + "'" + "{}".format(vsmart_pub_ip) + "'")
    print(vsmart_pub_ip_var)
with open(outfile_vars, 'a') as my_file:
    my_file.write(vsmart_pub_ip_var + "\n")


#Create a Secondary NIC and assign to the Lan subnet
#cmd_add_nic_router='aws ec2 create-network-interface --subnet-id' + " " + "{}".format(subnetid_lan) + " " + '--description "vsmart_nic"' + " " + '--groups' + " " + "{}".format(router_sg_id) + " " + '--private-ip-address 10.10.20.100'
outfile_add_vsmart_nic='add-vsmart-nic.json'
cmd_add_vsmart_nic='aws ec2 create-network-interface --region' + " " "{}".format(region) + " " + '--subnet-id' + " " + "{}".format(subnetid_lan) + " " + '--description "vsmart_nic_lan_sub"' + " " + '--groups' + " " + "{}".format(sgid) + " " + '--tag-specifications' + " " + "'ResourceType=network-interface,Tags=[{Key=Name,Value=" + "{}".format(name) + '}]'"" + "'"
output = check_output("{}".format(cmd_add_vsmart_nic), shell=True).decode().strip()
print("Output: \n{}\n".format(output))
with open(outfile_add_vsmart_nic, 'w') as my_file:
    my_file.write(output)

#Capture the secondary eni_id out of the json output
outfile_add_vsmart_nic='add-vsmart-nic.json'
with open(outfile_add_vsmart_nic) as access_json:
    read_content = json.load(access_json)
...
outfile_attach_vsmart_nic='attach-vsmart-nic.json'
cmd_attach_vsmart_nic='aws ec2 attach-network-interface --region' + " " + "{}".format(region) + " " +  '--network-interface-id' + " " + "{}".format(eni_id) + " " + '--instance-id' + " " + "{}".format(vsmart_instance_id) + " " + '--device-index 2'
output = check_output("{}".format(cmd_attach_vsmart_nic), shell=True).decode().strip()
print("Output: \n{}\n".format(output))
with open(outfile_attach_vsmart_nic, 'w') as my_file:
    my_file.write(output)

#Wait to check the instance is initialized
#Check that the instance is initialized
cmd_check_instance='aws ec2 wait instance-status-ok --instance-ids' + " " + vsmart_instance_id + " " + '--region' + " " + "{}".format(region)
output = check_output("{}".format(cmd_check_instance), shell=True).decode().strip()
print("Output: \n{}\n".format(output))

#assign an elastic ip to the secondary nic
#aws ec2 associate-address --allocation-id eipalloc-0f0d442ec6c89e9c8 --network-interface-id eni-1a2b3c4d
outfile_associate_eip_lan='associate_eip_lan.json'
cmd_associate_eip_lan='aws ec2 associate-address --allocation-id eipalloc-0f0d442ec6c89e9c8 --network-interface-id' + " " + eni_id
